...
class InboundEventReader {
    ...
    UUID payloadUuid(InboundEvent event, String fieldName) {
        return uuid(event.payload().path(fieldName).asText(null), event.aggregateId());
    }

    String payloadText(InboundEvent event, String fieldName) {
        String value = event.payload().path(fieldName).asText(null);
        return value == null || value.isBlank() ? null : value;
    }

    List<String> payloadStringList(InboundEvent event, String fieldName) {
        JsonNode node = event.payload().path(fieldName);
        if (!node.isArray()) {
            return List.of();
        }
